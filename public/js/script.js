/* -+-+-+--------------------------------------------+-+-+-
* Socket and jQuery Variable Declarations
 -+-+-+--------------------------------------------+-+-+- */
let socket = io.connect('http://localhost:3000');
let $tweetButton = $('#tweet-button');
let $tweetTextArea = $('#tweet-textarea');

/* -+-+-+--------------------------------------------+-+-+-
* Red box is user write over 140 characters
 -+-+-+--------------------------------------------+-+-+- */
$tweetTextArea.keyup(function() {
    if ($tweetTextArea.val().length > 140) {
        this.style.borderColor = 'red';
    } else {
        this.style.borderColor = '#aeddfa';
    }
});

/* -+-+-+--------------------------------------------+-+-+-
* Socket connection to update timeline
 -+-+-+--------------------------------------------+-+-+- */
socket.on('connect', function(data) {
    $tweetButton.on('click', function(e) {
        e.preventDefault();
        if ($tweetTextArea.val().length <= 140) {
            socket.emit('refreshData', $tweetTextArea.val());
            $tweetTextArea.val('');
        } else {
            alert('Tweet is greater than 140 characters, please adjust or start again...thank you!');
        }
    });
});

/* -+-+-+--------------------------------------------+-+-+-
* Prepend list item to timeline with updated tweet
 -+-+-+--------------------------------------------+-+-+- */
socket.on('messages', function(data) {
    let listElement = `<li>
            <strong class="app--tweet--timestamp">${data.timelineDate}</strong>
            ...
            <ul class="app--tweet--actions circle--list--inline">
              ...
              <li>
                <a class="app--retweet">
                  ...
                  <strong>${data.retweet_count}</strong>
                </a>
              </li>
              <li>
                <a class="app--like">
    ...
    $('.app--tweet--list').prepend(listElement);
});